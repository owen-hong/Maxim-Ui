...
CssSliceImgs.prototype = {
    ...
    getUploadImages : function(cssContent){
        var uploadImages = [];
        var regex = /background\-image\s*:\s*url\((.+\.png)\)/gi;
        var slice_regex = /^.+slice\/.+\.png/i;
        var matches = cssContent.match(regex);
        if(matches !== null && typeof  matches === 'object' && matches.length > 0){
            matches.forEach(function(match){
                //判断是否为slice下
                if(!slice_regex.test(match)){
                    var url = match.replace('background-image:url(','').replace(')','');
                    //如果结果数组中不存在该路径则添加到图片数组中，其实就是数组进行去重
                    if(uploadImages.indexOf(url) < 0){
