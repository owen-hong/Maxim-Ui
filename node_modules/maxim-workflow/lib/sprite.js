...
Sprite.prototype = {
    ...
    sprite : function(cssFiles,config,callback){
                ...
                new CssSliceImg().getImages(cssFiles[m],{},function(result){
                    ...
                    if(r.status === false){
                        ...
                        if(sceneType == 0){
                                ...
                                if(p !== 'count'){
                                    ...
                                    if(p === 'maxim_default'){
                                        if(sceneType == 1){
                                            ...
                                            props.push({
                                                name : path.basename(r.cssFile).replace('.css','') + '@2x',
                                                files : groups[p],
                                                ratio : sceneType
                                            });
                                        }
                            ...
                            /**
                             * 针对场景1去判断单倍图是否有对应的多倍图
                             * */
                            "use strict";
                            if(sceneType == 1){
                                //定义一个2倍图判断是否存在的map对象，避免文件二次判断
                                let png2xFlags = {},
                                    tmpGroup = [];
                                //构造待合并图片的groups对象
                                for(let i = 0 ; i < props.length ; i++){
                                    ...
                                    for(let j = 0 ; j < item.files.length ; j++){
                                        let fn = item.files[j],
                                        fn2x = fn.replace('.png','@2x.png');
                                        //单倍图对应存在多倍图
                                        if(fs.existsSync(fn2x)){
                                            tmp.push(fn2x);
                                            png2xFlags[fn2x] = true;
                                        }else{
                                            png2xFlags[fn2x] = false;
                                        }
                                    }
                                    //存在对应的多倍图文件数组
                                    if(tmp.length > 0){
                                ...
                                //将2倍图对应的item合并到groups
                                props = props.concat(tmpGroup);
                                /**
                                 * 处理css-slice-imgs返回的结果集，生成对应pngPathes2x，和pngMaps2x
                                 * 1、将一倍图的map直接赋值给2倍图的map
                                 * 2、去除屌2倍图map中的不存在的图片
                                 * 3、去除屌2倍图中不存在的映射
                                 */
                                r.pngMaps2x = r.pngMaps;
                                let pngMaps2x = {},
                                    pngs2x = [];
                                for(let key in r.pngMaps2x){
                                    //单倍图对应的二倍图是存在的
                                    let file2x = r.pngMaps2x[key].replace('.png','@2x.png');
                                    if( png2xFlags[file2x] == true){
                                        pngs2x.push(file2x);
                                        pngMaps2x[key] = file2x;
                                    }
                                }
                                //对混合模式下的多倍图图片数组和映射关系进行重新赋值
                                r.pngPathes2x = pngs2x;
                                r.pngMaps2x = pngMaps2x;
                            }

                            /**
                             * 根据props对应的标志来确定 ： ratio和padding
                             * */
                            for( var i = 0 ; i < props.length ; i++){
                                sprite_funcs.push(_folder(r,ret,relativePath,destSpritePath,buffer,props,i,sceneType));
                            }
                            /**
                             * 串行处理所有的图片合并需求
                             * */
                            async.waterfall(sprite_funcs,function(err,groupCss){
                                if(err){
                                    ...
                                    if(groupCss.status === false){
                                        ...
                                        var mediaContent = groupCss.mediaContent.split(common.getCurrentPlat.enter).join('');
                                        // var cssFileContent = groupCss.cssContent;
                                        var mediaCss = sceneType == 1 ? '@media only screen and (-webkit-min-device-pixel-ratio:1.25),only screen and (min-resolution:120dpi),only screen and (min-resolution:1.25dppx){' + mediaContent + '}' : '';
                                        cssFileContent = cssFileContent + mediaCss + cssTail;
                                        //生成最终的样式文件，并返回结果
        ...
        function _folder(r,ret,relativePath,destSpritePath,buffer,groups,n,ratio){
            ...
            var padding = taskData.ratio || ratio;

            /**
             * 关键判断：如果存在多倍图和单图图不一致的场景则认为是mix（混合场景）
             */
            var isMix = taskData.ratio !== ratio;
            ...
            if(n == 0){
                ...
                return function(cb){
                    spriteImages(r,ret,destSpritePath,{
                        ...
                        mediaContent : ''
                    },taskData,cb,padding,isMix);
                };
            }else{
                //返回非第一个函数的形式，会带有之前的函数处理的结果
                return function(prevResult , cb){
                    //处理之前传过来的结果
                    spriteImages(r,ret,destSpritePath,prevResult,taskData,cb,padding,isMix);
                };
            }
            /**
             * r ；图片抽取中的结果集
             ...
             * padding ：图片之间的间隙
             * */
            function spriteImages(r,ret,destSpritePath , prevData,taskData,cb,padding,isMix){
                ...
                var mediaData = prevData.mediaContent;
                    ...
                    if(err){
                        ...
                        fs.writeFile( fName , imgs.image ,function(err){
                            if(err){
                                ...
                                passData = generateCss(prevData.cssContent, r.rules, r[map_property],imgs.coordinates,imgs.properties,path.basename(fName),padding,isMix);
                                //添加到结果集中
                                ret.push({
                                    fName : fName.replace(config.destPath,'').replace(/\\/g,'\/'),
                                    status : true
                                ...
                                cb(null,{
                                    status : true,
                                    cssContent : passData.cssContent,
                                    mediaContent : mediaData + passData.mediaContent
                                });
                            }
                        });
                    }
         ...
         * ratio ：雪碧图倍率 默认为1
         * */
        function generateCss(content,rules,maps,coordinates,rect,spriteName,ratio,isMix){

            /**
             * 本函数一个比较重要的判断是
             * 情景1：完全单倍图、完全多倍图，则对源样式文件内容进行替换处理
             * 情景2：单倍图对应存在多倍图（混合模式），则传入的content不改变，只组装待处理的样式内容，以便后续添加到media query中
             * */
            ...
            for(var i = 0 ; i < rules.length ; i++){
                ...
                /**
                 * 考虑到规则中可能会有一些图片没有对应的2倍图，因此也要做判断。即判断下面2个条件
                 * 1、规则在maps中有对应的项
                 * 2、规则是否需要合并雪碧图的标志为true
                 * */
                if(maps[selector] != undefined && rules[i].isSprite === true) {
                    //这时要替换规则下的background-image、background-size、background-position
                    rules[i].rules['background-image'] = 'url(sprite/' + spriteName + ')';
                    //多倍图时设置background-size
                    if(spriteRatio > 1){
                        rules[i].rules['-webkit-background-size'] = rect.width / spriteRatio + 'px ' + rect.height / spriteRatio + 'px';
                        rules[i].rules['-moz-background-size'] = rect.width / spriteRatio + 'px ' + rect.height / spriteRatio + 'px';
            ...
            //混合模式下返回样式底部的media query的代码，否则返回处理后的css代码
            return {
                cssContent : content,
                mediaContent : cssMedia
            };
        }
        function confirmScene(r){
            ...
            if(pngPathes.length + pngPathes2x.length <= 0){
                //场景0：无待合并的雪碧图
                flag = 0;
            }else if(pngPathes2x.length > 0 && pngPathes.length <= 0){
                //场景2：移动端场景，只合并多倍图
                flag = 2;
            }else{
                //场景1：非移动端场景，这里会忽略掉@2x的写法，同时对单倍图进行判断是否存在对应的多倍图，如有则进行合并且声称相应的media query在最后面
                flag = 1;
            }
            return flag;
        }
