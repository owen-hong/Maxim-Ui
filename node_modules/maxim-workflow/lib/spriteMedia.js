...
class Media{
    ...
    init(cb){
        ...
        * */
        ...
        let vResult = csslint.verifyFile(this.file);
    ...
    createCss(results,fName){
                    ...
                    for(let i = 0 ; i < om.style.length ; i++){
                        ...
                        for(let m = 0 ; m < frameStyle.length ; m++){
                            if(m == frameStyle.length - 1){
                                if(frameStyle[m] == 'display' && /-webkit-box|-webkit-flex|flex/.test(frameStyle[frameStyle[m]])){
                                    frameText += classItem + 'display:-webkit-box;display:-wekbit-flex;display:flex';
                                }else{
                    ...
                    for(let k = 0 ; k < om.style.length ; k++){
                        ...
                        if(k == om.style.length - 1){
                            if(importants[property]){
                                ...
                                if(property == 'display' && /-webkit-box|-webkit-flex|flex/.test(om.style[property])){
                                    classItem = classItem + 'display:-webkit-box;display:-wekbit-flex;display:flex';
                                }else{
                            ...
                            if(importants[property]){
                                ...
                                if(property == 'display' && /-webkit-box|-webkit-flex|flex/.test(om.style[property])){
                                    classItem = classItem + 'display:-webkit-box;display:-webkit-flex;display:flex;';
                                }else {
                                    classItem = classItem + property + ':' + om.style[property] + ';';
                                }
                            }
        ...
        //判断是否添加样式时间戳
        if(_this.config.cssNameSwitch == 'true'){
        ...
        fs.writeFile(targetCssFile,cssContent,function (err) {
           ...
           //拷贝资源文件到目标地址
            ...
            if(resources.length > 0 && _this.config.resourceSyncSwitch == "true"){
                new mover().copy(resources,_this.config,function (data) {
    ...
    chargeMedia(media,fName){
        ...
        return function (cb) {
            ...
            let svgTester = new RegExp('\^\\s*url\\(\.\*' + sliceFeature + '\\/\.\+\\.svg\\)$','i');
            ...
            for(let i = 0 ; i < len ; i++){
                ...
                //获取选择器下待合并的图片
                if(rule.hasOwnProperty('background-image')){
                    let bgURL = rule['background-image'];

                    //svg sprite 图片提取
                    if(svgTester.test(bgURL)){
                        let absolutePath = path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,''));
                        ...
                        sliceMaps[absolutePath].push(rules[i].selectorText);
                    }else if(pngTester.test(bgURL)){
                        let absolutePath = path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,''));
                        ...
                        if(absolutePath.indexOf('@2x.png') >= 0){
                            ...
                            try{
                                if(fs.statSync(tmp).isFile()){
                                    ...
                                    sliceMaps[tmp] = rules[i].selectorText;
                                }
                            }catch (e){
                                //文件不存在
                            }
                        ...
                        sliceMaps[absolutePath].push(rules[i].selectorText);
                        //设置待合并的标志
                        rules[i].isSprite = true;
                    }else if(/^\s*url\(.+\)$/i.test(bgURL)){
                        //资源图地址
                        let resourceUrl = path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,''));
                        _this.resources.push(resourceUrl);
                    }
                    //避免background对它影响，要进行检测是否有background
                    if(rule.hasOwnProperty('background')){
                        //分析出background相关的属性及值  或者  直接提示这样写是有问题的
                    }
                }else if(rule.hasOwnProperty('background')){
                    let bgVal = rule['background'];
                    var regex = /url\([^\)]*\)/g;
                    var matches = bgVal.match(regex);

                    if(matches !== null && typeof  matches === 'object' && matches.length > 0){
                        for(let i = 0 ; i < matches.length ; i++){
                            _this.resources.push(path.resolve(rPath,matches[i].replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,'')));
                        }
                    }
                }
            }
            ...
            let promise = new Promise(function(resolve, reject) {
                if(svgSpriteImgs.length > 0) {
                    ...
                    let svgSpritePath = _this.config.spriteNameSwitch == "true" ? spritePath + media.name + '-' + _this.config.spriteName + '.svg' : spritePath + media.name + '.svg';
                    let svgSpritePngPath = _this.config.spriteNameSwitch == "true" ? spritePath + media.name + '-' + _this.config.spriteName + '-svg.png' : spritePath + media.name + '-svg.png';

                    svgSpriteInt.init(svgSpriteImgs, svgSpritePath, function (err, data) {
                        if (err) {
                            //将错误信息传递给最终的任务处理器
                            _this.rets.push({
                        ...
                        data.svgSpriteData.forEach(function (svgSpriteData) {
                            //单个小图的位置
                            let position = svgSpriteData.position.absolute;
                            ...
                            for (let i = 0; i < len; i++) {
                                if (includes(selector,rules[i].selectorText)) {
                                    ...
                                    rules[i].style['background'] = '-webkit-image-set(url(sprite/' + svgSpritePngFile + ')1x,url(sprite/' + svgSpriteFile + ') 2x)';
                                    rules[i].style['background-position'] = position.xy;
                                    rules[i].style['background-size'] = svgSpriteSize;
                                }
                            }
                        });
                        resolve();
                ...
                if (sliceImgs2x.length > 0) {
                    ...
                    Spritesmith.run({
                        ...
                        if (err) {
                            ...
                            //写图片文件，如果文件目录不存在则创建之
                            common.createPath(spritePath, fs);
                            let spriteFile = _this.config.spriteNameSwitch == "true" ? spritePath + media.name + '@2x-' + _this.config.spriteName + '.png' : spritePath + media.name + '@2x' + '.png';
                    ...
                    Spritesmith.run({
                        ...
                        if (err) {
                            ...
                            //写图片文件，如果文件目录不存在则创建之
                            common.createPath(spritePath, fs);
                            let spriteFile = _this.config.spriteNameSwitch == "true" ? spritePath + media.name + '-' + _this.config.spriteName + '.png' : spritePath + media.name + '.png';
                                    ...
                                    if (sliceImgs2xParse.length > 0) {
                                        ...
                                        Spritesmith.run({
                                            ...
                                            if (err) {
                                                ...
                                                let target = _this.config.spriteNameSwitch == "true" ? spritePath + media.name + '@2x-' + _this.config.spriteName + '.png' : spritePath + media.name + '@2x.png';
