    ...
    createCss(results,fName){
        ...
        fs.writeFile(targetCssFile,cssContent,function (err) {
           ...
           //拷贝资源文件到目标地址
            ...
            if(resources.length > 0 && (_this.config.resourceSyncSwitch == "true" || _this.config.cssNameSwitch == true)){
                //文件去重
                ...
                if(rule.hasOwnProperty('background-image')){
                    ...
                    if(svgTester.test(bgURL)){
                        ...
                        if(Array.isArray(bgURL)){
                            //遍历数组分别处理
                            for(let i = 0 ; i < bgURL.length ; i++){
                                let urls = bgURL[i].match(/url\([^\)]*\)/g);
                                if(urls != null && urls.length > 0){
                                    for(let j = 0 ; j < urls.length ; j++){
                                        _this.resources.push(path.resolve(rPath,urls[j].replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,'')));
                                    }
                                }
                            }
                        }else {
                            _this.resources.push(path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,'')));
                        }
                    }
