            ...
            _this.resources.forEach(function (r) {
                try{
                    if(fs.statSync(r).isFile()){
                        ...
                        _this.rets.push({
                            fName : r.replace(_this.config.localPath,''),
                            status : false,
                            message : '文件不存在'
                        });
                    }
                }catch(e){
                    if(r.indexOf('about:blank') >= 0){
                        _this.rets.push({
                            fName : r.replace(_this.config.localPath,''),
                            status : false,
                            message : '样式中有空背景引用'
        ...
        return function (cb) {
            ...
            for(let i = 0 ; i < len ; i++){
                ...
                if(rule.hasOwnProperty('background-image')){
                    ...
                    //svg sprite 图片提取
                    if(/^\s*url\(.+slice(\/|\\).+\.svg\)$/i.test(bgURL)){
                        let absolutePath = path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,''));
                        svgSpriteImgs.push(absolutePath);

                        //构建映射，单倍图和多倍图使用同一个映射
                        sliceMaps[absolutePath] = rules[i].selectorText;
                    }

                    if(/^\s*url\(.+slice(\/|\\).+\.png\)$/i.test(bgURL)){
                        let absolutePath = path.resolve(rPath,bgURL.replace(/^url\s*\(\s*/,'').replace(/\s*\)\s*$/,''));
