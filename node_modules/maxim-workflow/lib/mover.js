...
Copyfile.prototype = {
    ...
    copy : function(files,config,callback){
        ...
        var timestampSprite = config.timeStampSwitch && (config.imgSyncSwitch === 'true' || config.imgSyncSwitch == true) ? '-'+config.imgSyncName : '';
        ...
        function copyfile(fs,k){
            return function () {
                //创建目标路径，判断每一个文件的路径
                if(files[k].indexOf(config.destPath) >= 0){
                    var realDest = files[k].replace(path.basename(files[k]),'');
                    var rname = files[k];
                }else{
                ...
                try{
                    if(!fs.existsSync(files[k])){
                        results.push({
                            ...
                            status : false,
                            message : 'file is not existed'+ files[k]
                        });
                    }else{
                        if(files[k] !== rname){
